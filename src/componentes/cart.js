...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
          ...
          <div class="flex-grow-1">
            ...
            <div class="mt-2">
              <div class="btn-group btn-group-sm" role="group" aria-label="Product controls">
                <button type="button" class="btn btn-outline-secondary btn-decrease rounded-0" data-id="${
                  product.id
                }">-</button>
                <span class="btn btn-light disabled quantity-display rounded-0" data-id="${
                  product.id
                }">${product.quantity}</span>
                <button type="button" class="btn btn-outline-secondary btn-increase rounded-0" data-id="${
                  product.id
                }">+</button>
                <button type="button" class="btn btn-outline-danger ms-2 btn-remove rounded-0" data-id="${
                  product.id
                }">
  ...
  template += `
    ...
    <div class="mt-5 pt-3">
      ...
      <button id="btn-clear-cart" class="btn btn-danger w-100 btn-checkout">Eliminar todos los productos</button>
      <button id="btn-checkout" class="btn btn-primary w-100 btn-checkout mt-2">Finalizar compra</button>
    </div>
    `
...
function updateQuantityDisplay(productId, newQuantity) {
  const quantityDisplay = document.querySelector(
    `.quantity-display[data-id="${productId}"]`
  );
  if (quantityDisplay) {
  ...
  if (product) {
    const priceBadge = document.querySelector(
      `.list-group-item[data-product-id="${productId}"] .badge`
    );
    if (priceBadge) {
